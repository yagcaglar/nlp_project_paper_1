...
def read_data(dataset, style, max_len, prefix,
    ...
    src_file = '/content/drive/MyDrive/nlp_project/paper1/data/em/informal_ent_{}'.format(prefix)
    tgt_file = '/content/drive/MyDrive/nlp_project/paper1/data/em/formal_ent_{}'.format(prefix)
    src_file_2 = '/content/drive/MyDrive/nlp_project/paper1/data/em/informal_fam_{}'.format(prefix)
    tgt_file_2 = '/content/drive/MyDrive/nlp_project/paper1/data/em/formal_fam_{}'.format(prefix)


    src_seq, tgt_seq = [], []
    with open(src_file, 'r') as f1, open(tgt_file, 'r') as f2, open(src_file_2, 'r') as f3, open(tgt_file_2, 'r') as f4:
        f1 = f1.readlines()
        f2 = f2.readlines()
        f3 = f3.readlines()
        f4 = f4.readlines()
        ...
        index = [i for i in range(len(f3))]
        random.shuffle(index)
        index = index[:int(len(index) * ratio)]
        for i, (s, t) in enumerate(zip(f3, f4)):
            if i in index:
                s = tokenizer.encode(s)
                t = tokenizer.encode(t)
                s = s[:min(len(s) - 1, max_len)] + s[-1:]
