...
def main():
    ...
    model.load_state_dict(torch.load('/content/drive/MyDrive/nlp_project/paper1/checkpoints/bart_gyafc.chkpt'.format(opt.model, opt.dataset, opt.order, opt.style)))

    src_seq = []
    with open('/content/drive/MyDrive/nlp_project/paper1/data/em/informal_ent_test') as fin:
        for line in fin.readlines():
            src_seq.append(line.strip())

    with open('/content/drive/MyDrive/nlp_project/paper1/data/em/informal_fam_test') as fin:
        for line in fin.readlines():
            src_seq.append(line.strip())

    start = time.time()
    text =""
    with open('/content/drive/MyDrive/nlp_project/paper1/outputs/bart_gyafc.txt', 'w') as fout:
        for idx, line in enumerate(src_seq):
            ...
            generated_ids = model.generate(src.to(device),
                                           num_beams=5,
                                           max_length=30)
            a = [tokenizer.decode(g, skip_special_tokens=True,
