...
def main():
    ...
    cls.to(device).eval()
    cls.load_state_dict(torch.load('/content/pre-trained-formality-transfer/checkpoints/textcnn_{}.chkpt'.format(
                        opt.dataset)))
    ...
    for step in range(1, opt.steps):
        ...
        if ((len(train_loader) > opt.eval_step
            ...
            if eval_loss >= valid_loss:
                i=0
                torch.save(model.state_dict(), '/content/drive/MyDrive/nlp_project/paper1/checkpoints/{}_{}_{}_{}_{}.chkpt'.format(
                    opt.model, opt.dataset, opt.order, opt.style, i))
                i+=1
